...
Item {
  ...
  function valuesText() {
    ...
    if (displayMode === "primary")
      return primaryRemaining === null ? "—" : primaryRemaining + "%";
    if (displayMode === "secondary")
      return secondaryRemaining === null ? "—" : secondaryRemaining + "%";
  ...
  readonly property string panelText: panelStyle === "usage"
    ? valuesText()
    : "Codex " + valuesText()
  ...
  readonly property color statusColor: {
    if (fetchState === "error" || lowestRemaining <= 10)
      return Color.mError;
    if (lowestRemaining <= 25)
