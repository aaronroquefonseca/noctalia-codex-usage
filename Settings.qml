...
ColumnLayout {
  ...
  NLabel { label: "Refresh interval" }

  NSpinBox {
    from: 60
    to: 3600
    suffix: "s"
    Component.onCompleted: value = cfg.refreshIntervalSeconds ?? defaults.refreshIntervalSeconds
    onValueChanged: {
      if (!pluginApi)
        return;
  ...
  NLabel { label: "Codex executable path" }
